// src/components/Club/Football/Cricket.jsx
...
const Cricket = () => {
  ...
  const [cricketProducts, setCricketProducts] = useState([]);
  const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5000";

  // Fetch Cricket products from backend
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/products/get`);
        const cricketOnly = res.data.products.filter(p => p.category === "Cricket");
        setCricketProducts(cricketOnly);
      } catch (err) {
        console.error("Error fetching Cricket products:", err);
      }
    };
    fetchProducts();
  }, []);
  ...
  const handleNewProduct = (newProduct) => {
    if (newProduct.category === "Cricket") {
      setCricketProducts(prev => [newProduct, ...prev]);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setCricketProducts(prev => prev.filter(p => p._id !== id));
    ...
    <Layout>
      <div className="collection-container">
        ...
        {cricketProducts.length > 0 ? (
          ...
          <p>No Cricket products available.</p>
        )}
      </div>
    </Layout>
  );
