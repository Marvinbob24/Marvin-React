// src/components/Club/Football/ParisSaintGermain.jsx
...
const ParisSaintGermain = () => {
  const { user } = useContext(UserContext);
  const [psgProducts, setPsgProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const psgOnly = res.data.products.filter(p => p.category === "ParisSaintGermain");
        setPsgProducts(psgOnly);
      } catch (err) {
        console.error("Error fetching PSG products:", err);
      }
    };
    fetchProducts();
  }, []);
  ...
  const handleNewProduct = (newProduct) => {
    if (newProduct.category === "ParisSaintGermain") {
      setPsgProducts(prev => [newProduct, ...prev]);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setPsgProducts(prev => prev.filter(p => p._id !== id));
  ...
  return (
    <Layout>
      <div className="collection-container">
        <h2>Paris Saint Germain Collection</h2>

        {user?.isAdmin && (
          <ProductForm
            onProductAdded={handleNewProduct}
            defaultCategory="ParisSaintGermain"
            hideCategorySelect={true}
          />
        )}

        {psgProducts.length > 0 ? (
...
export default ParisSaintGermain;
