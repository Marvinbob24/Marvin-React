// src/components/Club/Football/Chelsea.jsx
...
const Chelsea = () => {
  ...
  const [chelseaProducts, setChelseaProducts] = useState([]);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const chelseaOnly = res.data.products.filter(p => p.category === "Chelsea");
        setChelseaProducts(chelseaOnly);
      } catch (err) {
        console.error("Error fetching Chelsea products:", err);
      }
    };
    fetchProducts();
  }, []);
  ...
  const handleNewProduct = (newProduct) => {
    if (newProduct.category === "Chelsea") {
      setChelseaProducts(prev => [newProduct, ...prev]);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setChelseaProducts(prev => prev.filter(p => p._id !== id));
  ...
  return (
    <Layout>
      <div className="collection-container">
        <h2>Chelsea Collection</h2>

        {user?.isAdmin && (
          <ProductForm
            onProductAdded={handleNewProduct}
            defaultCategory="Chelsea"
            hideCategorySelect={true}
          />
        )}

        {chelseaProducts.length > 0 ? (
          <div className="products-grid">
            {chelseaProducts.map(item => (
          ...
          <p>No Chelsea products available.</p>
        )}
      </div>
    </Layout>
  );
